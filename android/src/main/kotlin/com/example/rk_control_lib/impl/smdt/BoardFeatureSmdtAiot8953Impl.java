 ...
 * description :
 */
public class BoardFeatureSmdtAiot8953Impl extends BoardFeatureManager {
    String TAG = TagConstant.TAG_BOARD_FEATURE + "-SmdtAiot8953";
    private SmdtManager smdt;

    public BoardFeatureSmdtAiot8953Impl(Context context, String boardType, BoardControlListener boardControlListener) {
        setMContext(context);
        setMBoardType(boardType);
        setMBoardControlListener(boardControlListener);
        smdt = SmdtManager.create(getMContext());
        //每隔1s改变一次系统中的oom_adj参数,防止后台进程被杀
       // setAdjValueLoopThread  = new  SetAdjValueLoopThread(-1);
       // setAdjValueLoopThread.startLoop();
        log(TAG, "BoardFeatureSmdtAiot8953Impl 初始化完毕");
    }
...
//        log(TAG, "setPowerOffOn timeOffArray:" + timeOffArray.toString() + " timeOnArray:" + timeOnArray);
        ...
        log(TAG, "设置定时开关机(小时分钟) 视美泰 optime:" + optime + " cltime:" + cltime);
    }
